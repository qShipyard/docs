# Source: https://github.com/qShipyard/narratorlog/raw/refs/heads/main/README.md
...
export REDIS\_URL="redis://localhost:6379"
export APP\_URL="http://localhost:3000"
export APP\_SECRET="$(openssl rand -hex 32)"
export ENCRYPTION\_KEY="$(openssl rand -hex 32)"   # API and worker MUST share this value
...
In short: copy \`deploy/.env.example\` to \`.env\`, fill in every value (generate \`APP\_SECRET\` and
\`ENCRYPTION\_KEY\` with \`openssl rand -hex 32\` — the API and worker must share the same \`ENCRYPTION\_KEY\`),
