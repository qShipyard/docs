# Source: https://github.com/qShipyard/narratorlog/raw/refs/heads/main/deploy/SETUP.md
...
\`\`\`bash
openssl rand -hex 32   # use twice — once for APP\_SECRET, once for ENCRYPTION\_KEY
\`\`\`
...
\`DATABASE\_URL\` from your \`.env\`, so make sure that points at the \`postgres\` host
